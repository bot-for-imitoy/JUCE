...
public:
    ...
    std::optional<int> getRoutedOutputDeviceId() const noexcept override
    {
        const auto id = routedOutputDeviceId.load (std::memory_order_acquire);
        return id == oboe::kUnspecified ? std::optional<int>{} : id;
    }
